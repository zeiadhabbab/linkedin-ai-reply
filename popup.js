...
// Open persistent window so user can interact with page while UI remains open
if (document.getElementById("openWindow")) {
  document.getElementById("openWindow").onclick = () => {
    chrome.runtime.sendMessage({ action: "OPEN_PANEL" }, (res) => {
      if (res?.success) {
        console.log("Panel opened with window ID:", res.windowId);
      }
    });
  };
}
